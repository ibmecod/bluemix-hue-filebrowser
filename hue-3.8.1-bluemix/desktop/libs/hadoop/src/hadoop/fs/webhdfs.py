...
## Bluemix Apache Hadoop change - need Hadoop credentials
DEFAULT_HDFS_SUPERUSER_PASSWORD = desktop.conf.DEFAULT_HDFS_SUPERUSER_PASSWORD.get()
...
class WebHdfs(Hdfs):
  ...
  def _make_client(self, url, security_enabled, ssl_cert_ca_verify=True):
    ...
    ## Bluemix Apache Hadoop change - need basic auth with Hadoop credentials 
    client.set_basic_auth(DEFAULT_HDFS_SUPERUSER, DEFAULT_HDFS_SUPERUSER_PASSWORD)
...
#    }
  ...
  ## Bluemix Apache Hadoop change - remove doas param for WebHdfs requests
  def _getparams(self):
    return {
      "user.name" : WebHdfs.DEFAULT_USER
    }
